...
- `assets/` - исходные изображения и SVG-ресурсы интерфейса для встраивания в приложение
- `.github/workflows/` - сценарий автоматической сборки и релиза

## Поддерживаемые форматы

В текущей реализации приложение ищет в папке `music` файлы со следующими расширениями:
...
Этот список сейчас задан вручную в коде и используется как фильтр при сканировании папки и выборе файлов.
Поддержка воспроизведения всё равно зависит от возможностей `Qt Multimedia` и доступных кодеков в конкретной системе, поэтому теоретически Qt может уметь больше форматов, чем приложение сейчас ищет автоматически.
...
- Проект не имеет программного ограничения на количество участников, бан слушателей или защиту паролем.
...
- Windows-релиз из GitHub Actions собирается как portable-архив: он включает `Qt 6` DLL и основные `MinGW` runtime-библиотеки, поэтому обычно запускается без отдельно установленного Qt и без ручной установки компилятора.
- Linux-сборка не упакована в формат вроде `AppImage` или `.deb`, поэтому на другой системе может понадобиться установленный `Qt 6 runtime` и совместимые системные библиотеки.
...
- `Windows`: исполняемый файл, папка `music`, `Qt 6` DLL и `MinGW` runtime DLL для запуска на обычной Windows-системе без установленного Qt.
- `Linux`: исполняемый файл и папка `music`, но без полной контейнеризации зависимостей; для запуска на другой системе всё ещё может понадобиться установленный `Qt 6 runtime`.
