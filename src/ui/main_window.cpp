...
void MainWindow::buildUi() {
    ...
    importTracksButton_ = new QPushButton("Import tracks", libraryGroup);

    auto* libraryActionsLayout = new QHBoxLayout();
    libraryActionsLayout->setSpacing(8);
    libraryActionsLayout->addWidget(refreshLibraryButton_);
    libraryActionsLayout->addWidget(importTracksButton_);

    libraryList_ = new QListWidget(libraryGroup);
    libraryList_->setSelectionMode(QAbstractItemView::SingleSelection);
    libraryList_->setAlternatingRowColors(false);
    libraryList_->setSpacing(4);

    libraryLayout->addWidget(libraryTitle);
    libraryLayout->addWidget(libraryPathLabel_);
    libraryLayout->addLayout(libraryActionsLayout);
    ...
    connect(importTracksButton_, &QPushButton::clicked, this, &MainWindow::importTracks);
...
void MainWindow::importTracks() {
    QString targetDirectory = musicDirectory_;
    if (targetDirectory.isEmpty()) {
        switch (musicDirectoryMode_) {
            case MusicDirectoryMode::CurrentWorkingDirectory:
                targetDirectory = QDir::current().absoluteFilePath("music");
                break;
            case MusicDirectoryMode::ExecutableDirectory:
            case MusicDirectoryMode::Auto:
                targetDirectory = QCoreApplication::applicationDirPath() + "/music";
                break;
        }
    }

    if (targetDirectory.isEmpty()) {
        statusBar()->showMessage("Could not determine the music folder.");
        return;
    }

    QDir rootDir;
    if (!rootDir.mkpath(targetDirectory)) {
        statusBar()->showMessage("Failed to create the music folder.");
        return;
    }
    ...
    for (const QString& sourcePath : selectedFiles) {
        ...
        QString destinationPath = targetDir.filePath(candidateName);
        int suffixIndex = 1;
        ...
        while (QFileInfo::exists(destinationPath)) {
            ...
            candidateName = suffix.isEmpty()
                ? QString("%1_%2").arg(baseName).arg(suffixIndex)
                : QString("%1_%2.%3").arg(baseName).arg(suffixIndex).arg(suffix);
            destinationPath = targetDir.filePath(candidateName);
            ++suffixIndex;
        }
    ...
    if (importedCount > 0 && failedCount == 0) {
        statusBar()->showMessage(QString("Imported %1 track(s) into music.").arg(importedCount));
    } else if (importedCount > 0) {
        statusBar()->showMessage(
            QString("Imported %1 track(s), failed to import %2.").arg(importedCount).arg(failedCount)
        );
    } else {
        statusBar()->showMessage("No tracks were imported.");
    }
}
    ...
    for (const auto& track : playlist) {
        ...
        auto* titleLabel = new QLabel(
            !track.availableLocally
                ? QString("%1 [remote]").arg(QString::fromStdString(track.descriptor.title))
                : QString::fromStdString(track.descriptor.title),
            rowWidget
        );
        ...
        removeButton->setEnabled(hostMode_ && track.availableLocally && !track.localPath.isEmpty());
        connect(removeButton, &QToolButton::clicked, this, [this, localPath = track.localPath]() {
            if (localPath.isEmpty()) {
                return;
            }

            if (!QFile::remove(localPath)) {
                statusBar()->showMessage("Failed to remove track file.");
                return;
            }
